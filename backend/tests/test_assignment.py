...
def make_surveyor(**kwargs):
    s = {
        ...
        "schedule": {"shifts": [{"day": datetime.utcnow().strftime("%a")}], "on_call": False},
...
def test_score_zone_mismatch_returns_negative():
    ...
    score = _score_surveyor(s, "ZONE-RM-01", ["boundary_survey"], "normal", datetime.utcnow())
    assert score == -1


def test_score_increases_with_skill_and_capacity():
    s = make_surveyor()
    score = _score_surveyor(s, "ZONE-RM-01", ["boundary_survey", "gps_mapping"], "high", datetime.utcnow())
